...
public static class CairnDeprecationExtensions
{
    ...
    /// (RFC 9745 — <c>@unix-seconds</c>; when <paramref name="deprecatedAt"/> is omitted the registration time
    /// is used, since the final RFC defines no boolean form), a <c>Sunset</c> header (RFC 8594) when
    /// <paramref name="sunset"/> is given, and a
    ...
    /// <param name="deprecatedAt">When the endpoint became (or becomes) deprecated; emitted as the <c>Deprecation</c> header's date. Prefer supplying it — the fallback (the time this call runs, i.e. app startup) changes on every deployment.</param>
        ...
        where TBuilder : IEndpointConventionBuilder
    {
        ...
        // The boolean `true` form existed only in the draft and is invalid under the final RFC, so a call
        // without a date falls back to "deprecated as of this registration" rather than emitting it.
        var deprecation = string.Create(
            CultureInfo.InvariantCulture,
            $"@{(deprecatedAt ?? DateTimeOffset.UtcNow).ToUnixTimeSeconds()}");
