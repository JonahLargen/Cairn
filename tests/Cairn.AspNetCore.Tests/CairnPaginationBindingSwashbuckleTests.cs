...
public class CairnPaginationBindingSwashbuckleTests
{
    [Fact]
    public async Task Page_binding_documents_the_page_and_page_size_query_parameters()
    {
        ...
        Assert.Equal("integer", ParameterSchema(page).GetProperty("type").GetString());
    ...
    public async Task Cursor_binding_documents_the_cursor_and_limit_query_parameters()
    {
        ...
        Assert.Equal("string", ParameterSchema(cursor).GetProperty("type").GetString());

        var limit = Assert.Single(parameters, p => p.GetProperty("name").GetString() == "limit");
        Assert.Equal("integer", ParameterSchema(limit).GetProperty("type").GetString());
    }
    ...
    private static JsonElement ParameterSchema(JsonElement parameter) => parameter.GetProperty("schema");
}
