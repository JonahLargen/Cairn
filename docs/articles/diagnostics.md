...
| Computed but never emitted | Hypermedia was computed for instances that never reached the serializer — typically a deferred sequence (`IQueryable`, LINQ projection) inside `TypedResults.Ok(...)` whose re-enumeration produced fresh instances. Materialize (`ToList()`) before wrapping. |
