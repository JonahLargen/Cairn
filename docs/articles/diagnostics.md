...
| Deferred envelope items with no stable property | A pagination envelope exposes deferred items only through a property that recomputes a fresh sequence on each access (or a non-public member), so the buffered items can't be substituted at serialization; the sequence is enumerated twice and item links may be lost. A settable or init-only property works — materialize (`ToList()`) or expose a stable property otherwise. |
