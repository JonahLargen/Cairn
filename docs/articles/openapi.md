...
- **`WithPreconditions(...)`** — the operation gains a `412 Precondition Failed` response (as `application/problem+json`, with an `ETag` response header carrying the current validator to retry with) and, when `requireIfMatch: true`, a `428 Precondition Required` response — the statuses [`CairnPreconditions.Evaluate`](conditional-requests.md) returns for a conditional write. `Evaluate` runs in the handler body, so this marker is how the endpoint opts those responses into the document.
- **`WithDeprecation(...)`** — the operation is marked `deprecated: true`, so tooling flags it, and every response documents the headers the runtime emits: a `Deprecation` response header (RFC 9745), plus a `Sunset` header (RFC 8594) when a sunset date is supplied and a `Link` header with `rel="deprecation"` when a documentation URL is supplied. The headers land on every response — including the `304`/`412`/`428` the markers above add — because the runtime emits them regardless of status code.
