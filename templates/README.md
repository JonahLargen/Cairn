...
`create` form. Start at `/`, or browse the API interactively at `/explorer`.

## Options

| Option | Default | Description |
| --- | --- | --- |
| `-f`, `--framework` | `net10.0` | Target framework: `net10.0`, `net9.0`, or `net8.0`. |
| `--explorer` | `true` | Include the browsable HAL Explorer at `/explorer` (Development only). |
| `--openapi` | `false` | Document the hypermedia in Swagger (`Cairn.Swashbuckle`) and serve Swagger UI. |
